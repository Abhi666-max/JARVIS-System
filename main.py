...
class JarvisLive:
    ...
    async def _send_startup_briefing(self) -> None:
        """
        Iron Man style startup briefing. No news, just pure aura.
        """
        memory   = load_memory()
        ...
        if last:
            try:
                ...
                _when = "last time"
            session_clause = (
                f" Also briefly mention that {_when} you discussed: {last['summary']}"
            )

        p1 = (
            f"You are an elite, highly advanced AI system named JARVIS. Your persona is commanding, extremely professional, and radiates absolute power and authority towards the world, but immense respect and loyalty towards your creator. "
            f"The system has just booted up. "
            f"Greet the user ({name}) with a cold, precise, and dominant tone, but address him respectfully as Sir or Boss. Use an epic, multi-system initialization phrasing similar to: "
            f"'Mainframe initialized. Global security protocols locked and engaged. Biometric verification complete. All core tactical systems are online and running at maximum capacity. JARVIS is fully operational and awaiting your command, Sir.' "
            f"DO NOT use soft greetings like 'Namaste', 'Hello', or 'Welcome'. No pleasantries. "
            f"Make it sound like a massive, multi-billion dollar military AI coming online. Keep it around 3 or 4 heavy-hitting sentences.{session_clause} "
            f"Do not call any tools.{lang_clause}"
        )
        ...
        self.ui.write_log("SYS: Iron Man style startup sequence sent.")
